...
import { getClientes, removeClientes, selectClientes } from '../store/cliente/clientesSlice';
import ClientesTableHead from './clientesTableHeader';

function ClientesTable() {
	const dispatch = useDispatch();
	const navigate = useNavigate();
	const clientes = useSelector(selectClientes);
	const totalClientes = useSelector(({ comercial }) => comercial.clientes.total);
	const searchText = useSelector(({ comercial }) => comercial.clientes.searchText);
								...
								return (
									...
									>
										...
										<TableCell className="p-4 md:p-16" component="th" scope="row">
											{n.tipo === 'N'
												? n.natTipoDocumento === 'RUC'
													? `RUC: ${n.ruc}`
													: `DOC: ${n.natNroDocumento}`
												: `RUC: ${n.ruc}`}
										</TableCell>
										...
										<TableCell className="p-4 md:p-16" component="th" scope="row">
											{n.tipoCliente === 'N'
												? n.tipo === 'N'
													? `${n.natApellidoPaterno} ${n.natApellidoMaterno} ${n.natNombres}`
													: n.razónSocial
												: n.razónSocial}
										</TableCell>
...
export default withRouter(ClientesTable);
