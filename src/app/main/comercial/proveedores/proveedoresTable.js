...
import {
	...
	removeProveedores,
	selectProveedores,
} from '../store/proveedor/proveedoresSlice';
import ProveedoresTableHead from './proveedoresTableHeader';

function ProveedoresTable(props) {
	const dispatch = useDispatch();
	const navigate = useNavigate();
	const proveedores = useSelector(selectProveedores);
	const totalProveedores = useSelector(({ comercial }) => comercial.proveedores.total);
	const searchText = useSelector(({ comercial }) => comercial.proveedores.searchText);
	...
	function handleClick(item) {
		navigate(`/comercial/proveedores/${item.id}`);
	}
		...
		<div className="w-full flex flex-col">
							...
							.map(n => {
								...
								return (
									<TableRow className="h-72 cursor-pointer" hover tabIndex={-1} key={n.id}>
										...
										<TableCell className="p-4 md:p-16" component="th" scope="row">
											{
												...
												n.tipo === 'J'
													? n.tipo === 'J'
														? n.razonSocial
														: `${n.apellidoPaterno} ${n.apellidoMaterno} ${n.nombres}`
													: `${n.apellidoPaterno} ${n.apellidoMaterno} ${n.nombres}`
											}
										</TableCell>
										<TableCell className="p-4 md:p-16" component="th" scope="row">
										...
										<TableCell className="p-4 md:p-16" component="th" scope="row">
											{
												...
												n.tipo === 'J' ? (
													...
													<>
														{n.ruc ? (
															<div style={{ paddingBottom: '6px' }}>RUC: {n.ruc} </div>
														) : null}
														{n.nroDocumento && parseFloat(n.nroDocumento) > 0 ? (
															<div>DOC: {n.nroDocumento}</div>
														) : null}
													</>
												)
											}
										...
										<TableCell className="p-4 md:p-16" component="th" scope="row">
											{n.tipo === 'J' ? 'Persona Jurídica' : 'Persona Natural'}
										</TableCell>
										<TableCell className="p-4 md:p-16" component="th" scope="row">
											{n.celular}
										</TableCell>
			...
			{openModalDelete && (
				<ModalConfirmDelete
					...
					register={itemDelete?.ruc || itemDelete?.nroDocumento || null}
				/>
			)}
		</div>
	);
...
export default withRouter(ProveedoresTable);
