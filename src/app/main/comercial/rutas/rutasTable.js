...
import { getRutas, removeRutas, selectRutas } from '../store/ruta/rutasSlice';
import RutasTableHead from './rutasTableHeader';

function RutasTable(props) {
	const dispatch = useDispatch();
	const navigate = useNavigate();
	const rutas = useSelector(selectRutas);
	const totalRutas = useSelector(({ comercial }) => comercial.rutas.total);
	const searchText = useSelector(({ comercial }) => comercial.rutas.searchText);
	...
	function handleClick(item) {
		navigate(`/comercial/rutas/${item.id}`);
	}
...
export default withRouter(RutasTable);
