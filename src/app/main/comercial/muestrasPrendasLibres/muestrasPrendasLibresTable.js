...
import MuestrasPrendasLibresTableHead from './muestrasPrendasLibresTableHeader';

import {
	cambiarEstadoMuestraPrenda,
	getMuestrasPrendasLibres,
	removeMuestrasPrendasLibres,
	selectMuestrasPrendasLibres,
} from '../store/muestraPrendaLibre/muestrasPrendasLibresSlice';
import ModalConfirmarEstado from './modalConfirmarEstado';

function MuestrasPrendasLibresTable(props) {
	const dispatch = useDispatch();
	const navigate = useNavigate();
	const muestrasPrendasLibres = useSelector(selectMuestrasPrendasLibres);
	...
	const totalMuestras = useSelector(({ comercial }) => comercial.muestrasPrendasLibres.total);
	const searchText = useSelector(({ comercial }) => comercial.muestrasPrendasLibres.searchText);

	const [loading, setLoading] = useState(true);
	const [selected, setSelected] = useState([]);
	const [data, setData] = useState(muestrasPrendasLibres);
	...
	const cambiaEstado =
		modulos.findIndex(modulo => modulo.nombre === 'encargadoMuestraPrendaLibre') !== -1;
		...
		return (
			...
			>
				<Typography color="textSecondary" variant="h5">
					No hay muestras de prendas libres!
				</Typography>
			</motion.div>
		);
	}
	...
	const cambiarEstado = muestra => {
		showToast(
			...
			'update',
			'Estado Muestra de Prenda Libre'
		);
	};

	const onClickEditar = item => {
		navigate(`/comercial/muestrasPrendasLibres/${item.id}?action=ET`);
	};

	const onClickVisualizar = item => {
		navigate(`/comercial/muestrasPrendasLibres/${item.id}?action=SW`);
	};
		...
		<div className="w-full flex flex-col">
			<FuseScrollbars className="flex-grow overflow-x-auto">
				<Table stickyHeader className="min-w-xl" aria-labelledby="tableTitle">
					<MuestrasPrendasLibresTableHead
						page={page}
...
export default withRouter(MuestrasPrendasLibresTable);
