...
import DesarrolloColorHiloHeader from './desarrolloColorHiloHeader';

import reducer from '../../store';
import {
	getDesarrolloColorHiloId,
	newDesarrolloColorHilo,
	resetDesarrolloColorHilo,
...
const DesarrolloColorHilo = () => {
	...
	const [currentTipoHilo, setCurrentTipoHilo] = useState('');
		...
		await showToast(
			{
				promesa: async () => {
					...
					} = await httpClient.get(`comercial/desarrollo-color-hilo/${id}`);
					if (statusCode === 200) {
						...
						setCurrentTipoHilo(body.tipoHilo);
					}
					setDisabled(false);
	...
	useDeepCompareEffect(() => {
		(() => {
			if (routeParams.id === 'nuevo') {
				dispatch(newDesarrolloColorHilo());
			} else {
				dispatch(getDesarrolloColorHiloId(routeParams.id)).then(action => {
					if (!action.payload) {
						setNoExiste(true);
					}
				});
	...
	useEffect(() => {
		return () => {
			dispatch(resetDesarrolloColorHilo());
			setNoExiste(false);
		};
	}, [dispatch]);
				...
				<div className="p-16 sm:p-24 ">
					...
					<div>
						...
						<div className="flex flex-col sm:flex-row mr-24 sm:mr-4">
							<InputString
								label="Tipo de Hilo"
								value={currentTipoHilo}
								onChange={event => {
									setCurrentTipoHilo(event.target.value);
								}}
								disabled={disabled}
							/>
							<InputString
...
export default withReducer('comercial', reducer)(DesarrolloColorHilo);
