...
import { getPrendas, removePrendas, selectPrendas } from '../store/prenda/prendasSlice';
import PrendasTableHead from './prendasTableHeader';

function PrendasTable(props) {
	const dispatch = useDispatch();
	const navigate = useNavigate();
	const prendas = useSelector(selectPrendas);
	const totalPrendas = useSelector(({ maestros }) => maestros.prendas.total);
	const searchText = useSelector(({ maestros }) => maestros.prendas.searchText);

	const [loading, setLoading] = useState(true);
	const [selected, setSelected] = useState([]);
	const [data, setData] = useState(prendas);
	...
	function handleClick(item) {
		navigate(`/maestros/prendas/${item.id}`);
	}
	...
	if (data.length === 0) {
		return (
			...
			>
				<Typography prenda="textSecondary" variant="h5">
					No hay prendas!
				</Typography>
			</motion.div>
		);
							...
							.map(n => {
								...
								return (
									...
									>
										...
										<TableCell className="p-4 md:p-16" component="th" scope="row">
											{n.familiaPrenda.descripcion}
										</TableCell>

										<TableCell className="p-4 md:p-16" component="th" scope="row">
											{n.unidadMedida.nombre}
										</TableCell>
										<TableCell
...
export default withRouter(PrendasTable);
