...
import FamiliaEmpresaHeader from './familiaTelaHeader';
...
const FamiliaTela = () => {
	...
	const familiaTela = useSelector(({ maestros }) => maestros.familiaTela);
	...
	useDeepCompareEffect(() => {
		function updateFamiliaState() {
			...
			if (id === 'nuevo') {
				 ...
				 */
				dispatch(newFamiliaTela());
			} else {
				/**
				 * Get Empresa data
				 */
				dispatch(getFamiliaTelaId(id)).then(action => {
					/**
		...
		return (
			...
			>
				<Typography color="textSecondary" variant="h5">
					No se encontro la familia de tela!
				</Typography>
				<Button
					className="mt-24"
					component={Link}
	...
	if (
		_.isEmpty(form) ||
		(familiaTela && parseInt(routeParams.id, 10) !== familiaTela.id && routeParams.id !== 'nuevo')
	) {
...
export default withReducer('maestros', reducer)(FamiliaTela);
