...
import { getColores, removeColores, selectColores } from '../store/color/colorsSlice';
import ColoresTableHead from './coloresTableHeader';

function ColoresTable(props) {
	const dispatch = useDispatch();
	const navigate = useNavigate();
	const colores = useSelector(selectColores);
	const totalColores = useSelector(({ maestros }) => maestros.colores.total);
	const searchText = useSelector(({ maestros }) => maestros.colores.searchText);

	const [loading, setLoading] = useState(true);
	const [selected, setSelected] = useState([]);
	const [data, setData] = useState(colores);
	...
	function handleClick(item) {
		navigate(`/maestros/colores/${item.id}`);
	}
							...
							.map(n => {
								...
								return (
									...
									>
										...
										{/* <TableCell className="p-4 md:p-16" component="th" scope="row">
											{n.tipoDesarrollo === 'HILO'
												? 'Hilo Color'
												: n.tipoDesarrollo === 'TELA'
												? 'Tela Cruda'
												: 'No se encuentra el tipo'}
										</TableCell> */}

										<TableCell className="p-4 md:p-16" component="th" scope="row">
											{n.alternativas[0]?.nombre || '-'}
										</TableCell>
										...
										>
											<Avatar
												variant="rounded"
												src={baseUrl + n.alternativas[0].imagenUrl}
												sx={{ width: 56, height: 56 }}
												onClick={() => {
													if (n.alternativas[0].imagenUrl) {
														setImagenActiva(baseUrl + n.alternativas[0].imagenUrl);
													}
												}}
											/>
											<Avatar
												variant="rounded"
												src={baseUrl + n.alternativas[0].imagenUrl2}
												sx={{ width: 56, height: 56 }}
												onClick={() => {
													if (n.alternativas[0].imagenUrl2) {
														setImagenActiva(baseUrl + n.alternativas[0].imagenUrl2);
													}
												}}
											/>
										</TableCell>
...
export default withRouter(ColoresTable);
