...
import { getLavados, removeLavados, selectLavados } from '../store/lavados/lavadosSlice';
import TitulacionesTableHead from './lavadosTableHeader';

function LavadosTable(props) {
	const dispatch = useDispatch();
	const navigate = useNavigate();
	const lavados = useSelector(selectLavados);
	const totalLavados = useSelector(({ maestros }) => maestros.lavados.total);
	const searchText = useSelector(({ maestros }) => maestros.lavados.searchText);

	const [loading, setLoading] = useState(true);
	const [selected, setSelected] = useState([]);
	const [data, setData] = useState(lavados);
	...
	function handleClick(item) {
		navigate(`/maestros/lavados/${item.id}`);
	}
...
export default withRouter(LavadosTable);
