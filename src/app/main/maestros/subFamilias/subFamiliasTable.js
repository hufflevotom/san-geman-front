...
import {
	...
	removeSubFamilias,
	selectSubFamilias,
} from '../store/sub-familia/SubFamiliasSlice';
import SubFamiliasTableHead from './subFamiliasTableHeader';

function SubFamiliasTable(props) {
	const dispatch = useDispatch();
	const navigate = useNavigate();
	const subfamilias = useSelector(selectSubFamilias);
	const totalSubfamilias = useSelector(({ maestros }) => maestros.subfamilias.total);
	const searchText = useSelector(({ maestros }) => maestros.subfamilias.searchText);

	const [loading, setLoading] = useState(true);
	const [selected, setSelected] = useState([]);
	const [data, setData] = useState(subfamilias);
	...
	function handleClick(item) {
		navigate(`/maestros/subfamilias/${item.id}`);
	}
...
export default withRouter(SubFamiliasTable);
