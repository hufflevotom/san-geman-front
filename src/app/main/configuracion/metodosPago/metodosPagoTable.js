...
import {
	...
	removeMetodosPago,
	selectMetodosPago,
} from '../store/metodoPago/metodosPagoSlice';
import MetodosPagoTableHead from './metodosPagoTableHeader';

function MetodosPagoTable(props) {
	const dispatch = useDispatch();
	const navigate = useNavigate();
	const metodosPago = useSelector(selectMetodosPago);
	const totalMetodosPago = useSelector(({ configuraciones }) => configuraciones.metodosPago.total);
	const searchText = useSelector(({ configuraciones }) => configuraciones.metodosPago.searchText);

	const [loading, setLoading] = useState(true);
	const [selected, setSelected] = useState([]);
	const [data, setData] = useState(metodosPago);
	...
	function handleClick(item) {
		navigate(`/configuracion/metodos-pago/${item.id}`);
	}
...
export default withRouter(MetodosPagoTable);
