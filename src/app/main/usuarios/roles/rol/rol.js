...
import { getRolId, newRol, resetRol } from '../../store/rol/rolSlice';
...
const Rol = () => {
	...
	const user = useSelector(({ usuarios }) => usuarios.rol);
	...
	useDeepCompareEffect(() => {
		function updateRolState() {
			const { id } = routeParams;
			if (id === 'nuevo') {
				dispatch(newRol());
			} else {
				dispatch(getRolId(id)).then(action => {
					if (!action.payload) {
	...
	if (noExisteUsuario) {
		return (
			...
			>
				...
				<Button className="mt-24" component={Link} variant="outlined" to="/roles" color="inherit">
					Regresar
				</Button>
			</motion.div>
		);
	...
	return (
		<FormProvider {...methods}>
			<Root
				header={<RolHeader tipo={routeParams.id} />}
				// contentToolbar={
				// 	<Tabs
				// 		value={tabValue}
...
export default withReducer('usuarios', reducer)(Rol);
