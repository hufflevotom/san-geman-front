...
import { createSalida, updateSalida } from '../../../store/almacenAvio/salidas/salidaAvioSlice';
	...
	async function saveAlmacenAvio() {
		try {
			...
			if (
				...
				const arrData = [];
				body.avios.forEach(detalle => {
					if (parseFloat(detalle.cantidad) <= parseFloat(detalle.avio?.cantidad)) {
						arrData.push({
							cantidad: parseFloat(detalle.cantidad),
							//* Unidad Kilogramos
							unidadId: detalle.avio?.unidad?.id,
							producto: {
								avioId: detalle.avio?.producto?.avio?.id,
								tallaId: detalle.avio?.producto?.talla?.id,
							},
						});
					} else {
						throw new Error(
							`La cantidad de ${detalle.avio?.producto?.avio?.nombre} no puede ser mayor a ${detalle.avio?.cantidad}`
						);
					}
				});
				if (
				...
				body.detallesProductosSalidasAlmacenesAvios = arrData;
				// } else {
			...
			if (tipo === 'nuevo') {
				...
				navigate(`/almacen/avios/salidas`);
			} else {
