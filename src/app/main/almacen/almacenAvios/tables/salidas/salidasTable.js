...
import {
	getSalidas,
	selectSalidas,
} from 'app/main/almacen/store/almacenAvio/salidas/salidasAviosSlice';
...
function SalidasTable({ setOpenModal, setDataModal, setOpenAnular, setCallBackAuth }) {
	...
	const totalSalidas = useSelector(({ almacen }) => almacen.salidasAvio.total);
	const searchText = useSelector(({ almacen }) => almacen.salidasAvio.searchText);
					...
					<TableBody>
						{_.orderBy(
							...
							.map(n => {
								let totalCantidad = 0;
								let unidad = '';
								(n.detallesProductosSalidasAlmacenesAvios || []).forEach((detalle, index) => {
								...
								return (
									...
									>
										...
										{/* <TableCell className="p-4 md:p-16" component="th" scope="row" align="center">
											{n.ordenServicioCorte ? n.ordenServicioCorte?.codigo : 'No tiene orden'}
										</TableCell> */}
										<TableCell className="p-4 md:p-16" component="th" scope="row" align="center">
											{n.nNota}
										</TableCell>
