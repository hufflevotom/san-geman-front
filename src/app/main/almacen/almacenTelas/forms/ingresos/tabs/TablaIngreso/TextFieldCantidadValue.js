...
const TextFieldCantidadValueTable = ({
	label,
	dataSeleccionada,
	row,
	onChangeTable,
	...
	const traerUnidades = async busqueda => {
		...
		const arrayFiltrado = [];
		arrayFiltrado.push(data.filter(unidad => unidad.id === row.producto.tela.unidadMedida.id)[0]);
		arrayFiltrado.push(
			data.filter(unidad => unidad.id === row.producto.tela.unidadMedidaSecundaria.id)[0]
		);

		//* arrayFiltrado que no repita unidades
		const arrayFiltradoSinRepetir = [];
		arrayFiltrado.forEach(unidad => {
			if (!arrayFiltradoSinRepetir.some(e => e.id === unidad.id)) {
				arrayFiltradoSinRepetir.push(unidad);
			}
		});

		setDataUnidades(
			arrayFiltradoSinRepetir.map(unidad => ({
				...unidad,
				label: unidad.nombre,
			}))
		);
...
export default TextFieldCantidadValueTable;
